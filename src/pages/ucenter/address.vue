...
<view class="container">
    <view class="address-list" v-if="addressList.length">
        <view class="item" v-for="(item, index) of addressList" :key="item.id" @click="update"
          :data-address-id="item.id" :data-index="index">
            ...
            <view class="c">
                ...
                <view class="address">{{item.provinceNm + item.cityNm + item.districtNm + item.address}}</view>
            </view>
            <view class="r">
    ...
    <view class="empty-view" v-if="addressList.length <= 0">
      <image class="icon" src="/static/images/address_no.png"/>
      <text class="text">收货地址在哪里</text>
    </view>
    <view class="add-address"  @click="add" data-address-id="0">新建</view>
</view>
</template>
      ...
      wx.showModal({
        ...
        success: async function (res) {
          if (res.confirm) {
            ...
            const res = await api.deleteAddress({ id: addressId });
            if (res.rspCd === '00000') {
              wx.showToast({
                title: '删除成功'
              })
              this_.getAddressList();
            }
          }
        }
      })
...
.address-list{
  ...
  background: #fff url(~@/../static/images/address_bg_bd.png) 0 0 repeat-x;

...
<div>
    ...
    <swiper class="goodsimgs" indicator-dots="true" autoplay="true" interval="3000" duration="1000">
        ...
        <img :src="item.imgUrl" background-size="cover"/>
        </swiper-item>
    </swiper>
    ...
    <view class="comments" v-if="comment.count > 0">
        ...
        <view class="item">
            ...
            <view class="time">{{comment.commentInfo.createTime}}</view>
            </view>
            <view class="content">
            {{comment.commentInfo.content}}
            </view>
            <view class="imgs" v-if="comment.commentInfo.picUrlList.length">
            <image class="img" v-for="(item, index) of comment.commentInfo.picUrlList" :key="item.id" :data-index="index" :src="item"/>
            </view>
    ...
    <view class="goods-attr">
        ...
        <view class="l">
        <view class="item" v-for="(item, index) of attributeList" :key="item.name" :data-index="index">
    ...
    <view class="attr-pop">
        ...
        <view class="img-info">
        <img class="img" :src="gallery[0].imgUrl"/>
        <view class="info">
            <view class="c">
        ...
        <view class="spec-con">
        <view class="spec-item" v-for="(item, index) of specList" :key="item.id" :data-index="index">
            <view class="name">{{item.name}}</view>
            <view class="values">
                <view :class="iitem.checked ? 'selected value' : 'value'" @click="clickSkuValue" v-for="(iitem, iindex) of item.valueList" :key="iitem.id" :data-value-id="iitem.id" :data-index="iindex" :data-name-id="iitem.specId">{{iitem.value}}</view>
            </view>
        </view>
...
export default {
  ...
  methods: {
    ...
    async getGoodsInfo () {
      ...
      if (res.rspCd === '00000') {
        debugger
        this.goods = res.goodsInfoPo
        this.brand = res.brandInfoPo
        this.gallery = res.galleryList.length > 0 ? res.galleryList : [{ imgUrl: '/static/images/icon_error.png' }]
        this.attributeList = res.attributeList
        this.issueList = res.issueList
        this.comment = res.comment
        this.packageSpecContent(res.specList)
        this.productList = res.productList
        let collectInfoPo = res.collectInfoPo
        if (collectInfoPo) {
          this.userHasCollect = collectInfoPo.isCollect
    ...
    // 组装商品规则内容
    packageSpecContent (specList) {
      let specificationList = []
      let hasSpecificationList = {}
      // 按规则名称分组
      for (let i = 0; i < specList.length; i++) {
        const specItem = specificationList[i]
        if (!hasSpecificationList[specItem.specId]) {
          ...
          for (let j = 0; j < specificationList.length; j++) {
            if (specificationList[j].specId === specItem.specId) {
              specificationList.valueList.push(specItem)
            }
          }
        }
      }

      this.specList = specificationList
    },
    // 获得“相关商品推荐”信息
    ...
    clickSkuValue (event) {
      ...
      let _specList = this.specList
      for (let i = 0; i < _specList.length; i++) {
        if (_specList[i].specId === specNameId) {
...
.service-policy .item {
  background: url(~@/../static/images/policy_red.png) 0 center no-repeat;
...
.goods-info .brand text {
  ...
  color: #f48f18;
  border-radius: 4px;
  background: url(~@/../static/images/arrow_detail.png) 95% center no-repeat;
...
.comments .h .i {
  ...
  height: 100.5rpx;
  line-height: 100.5rpx;
  background: url(~@/../static/images/address_right.png) right center no-repeat;
